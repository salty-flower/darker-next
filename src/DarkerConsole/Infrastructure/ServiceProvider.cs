...
internal partial class ServiceProvider
{
    ...
    {
        return LoggerFactory.Create(builder => builder.AddConsole()).CreateLogger<ThemeService>();
    }

    private static ILogger<ToastService> CreateToastServiceLogger()
    {
        return LoggerFactory.Create(builder => builder.AddConsole()).CreateLogger<ToastService>();
    }

    private static ILogger<TrayCommand> CreateTrayCommandLogger()
    {
        return LoggerFactory.Create(builder => builder.AddConsole()).CreateLogger<TrayCommand>();
    }
